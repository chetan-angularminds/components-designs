...
export function CTASection() {
  return (
    <section className="py-20 bg-gradient-to-r from-orange-600 via-amber-600 to-red-600 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 text-center relative">
          ...
          >
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                size="lg"
                variant="secondary"
                className="text-lg px-8 bg-white text-orange-600 hover:bg-orange-50 group"
              >
                Get Started Today
        ...
        />
        <motion.div
          className="absolute bottom-10 right-10 w-16 h-16 bg-orange-300/20 rounded-full"
          animate={{ y: [0, 20, 0] }}
          transition={{ duration: 4, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: 1 }}
        />
        <motion.div
          className="absolute top-1/4 right-1/4 w-8 h-8 bg-amber-200/15 rounded-full"
          animate={{ y: [0, -15, 0], x: [0, 10, 0] }}
          transition={{ duration: 5, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: 0.5 }}
        />
        <motion.div
          className="absolute bottom-1/3 left-1/4 w-12 h-12 bg-white/8 rounded-full"
          animate={{ y: [0, 25, 0] }}
          transition={{ duration: 6, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: 2 }}
        />
        <motion.div
          className="absolute top-1/2 left-1/6 w-6 h-6 bg-orange-200/20 rounded-full"
          animate={{ y: [0, -10, 0], x: [0, -8, 0] }}
          transition={{ duration: 4.5, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: 1.5 }}
        />
        <motion.div
          className="absolute bottom-1/4 right-1/6 w-10 h-10 bg-slate-200/12 rounded-full"
          animate={{ y: [0, 18, 0] }}
          transition={{ duration: 3.5, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: 0.8 }}
        />
        <motion.div
          className="absolute top-3/4 left-1/3 w-7 h-7 bg-amber-300/18 rounded-full"
          animate={{ y: [0, -12, 0], x: [0, 5, 0] }}
          transition={{ duration: 5.5, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: 2.5 }}
        />
        <motion.div
          className="absolute top-1/6 right-1/3 w-9 h-9 bg-white/12 rounded-full"
          animate={{ y: [0, 22, 0] }}
